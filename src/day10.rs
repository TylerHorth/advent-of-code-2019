...
fn parse_map(input: &str) -> Vec<Vec<bool>> {
    separated_list(line_ending, many1(asteroid))(input).unwrap().1
}
...
fn print_map(map: &Vec<Vec<bool>>, term: &mut Term, x: i32, y: i32, laser: &(usize, usize), found: &HashSet<(i32, i32)>) {
    for j in 0..map.len() {
        for i in 0..map[0].len() {
            if laser.0 == i && laser.1 == j {
                write!(term, "{}", style("X").cyan()).unwrap();
            } else if i == x as usize && j == y as usize {
                write!(term, "{}", style("#").red()).unwrap();
            } else if found.contains(&(i as i32, j as i32)) {
                write!(term, "{}", style("#").green()).unwrap();
            } else if map[j][i] {
                write!(term, "#").unwrap();
            } else {
                write!(term, ".").unwrap();
...
fn find_200th(map: &Vec<Vec<bool>>) -> i32 {
    ...
    print_map(map, &mut term, laser.0 as i32, laser.1 as i32, &laser, &found);
        ...
        loop {
            ...
            match ast {
                Some(true) if !found.contains(&(x, y)) => {
                    term.move_cursor_up(map.len()).unwrap();
